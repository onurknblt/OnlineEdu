...
namespace OnlineEdu.WebAPI.Controllers
{
    ...
    public class CoursesController(IGenericService<Course> _courseService, IMapper _mapper) : ControllerBase
    {
        ...
        {
            var createdCourses = _mapper.Map<Course>(createCourseDto);
            _courseService.TCreate(createdCourses);
        ...
        {
            var updatedCourses = _mapper.Map<Course>(updateCourseDto);
            _courseService.TUpdate(updatedCourses);
